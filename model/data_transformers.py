...
mapper = DataFrameMapper([
    ...
    ('policy_code', sklearn.preprocessing.LabelBinarizer())], input_df=True, df_out=True)
...
valid_purpose_cats = ['debt consolidation',
                      'credit card',
                      'other',
                      'home improvement',
                      'major purchase',
...
def clean_data(df):
    ...
    df['purpose_cat'] = [p if p in valid_purpose_cats else 'other' for p in df['purpose_cat']]
...
class RawDataTransformer:
    ...
    def fit_transform(self, df):
        X, y = clean_data(df)
        return self.mapper.fit_transform(X), y
